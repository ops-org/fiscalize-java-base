...
public class NotaFiscalDao extends HibernateGenericDao<NotaFiscal, Integer> {
	...
	public NotaFiscal pegarRandomica(PedidoNota pedidoNota, int usuarioId) {
		...
		String sql = "SELECT notaFiscalId FROM NotaFiscal JOIN (SELECT CEIL(RAND() * (SELECT MAX(notaFiscalId) FROM NotaFiscal WHERE NotaFiscal.valor>50)) AS id) AS Random WHERE NotaFiscal.valor>50 AND NotaFiscal.notaFiscalId >= Random.id LIMIT 1";
        ...
        if(data.size()>0) {
            ...
            if(notaFiscalId!=null) {
            	...
            	criteria.add(Restrictions.sqlRestriction("valor>50 ORDER BY RAND()")); // se não pegou notaFiscalId, pega aleatório de maneira lenta
            }
